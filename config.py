...
#######################################################################
# the list you can do
model_list = ['DCCRN', 'CRN', 'FullSubNet', 'DCCRN+CM']
loss_list = ['MSE', 'SDR', 'SI-SNR', 'SI-SDR']
perceptual_list = [False, 'LMS', 'PMSQE'] # 지각적 성능 향상 목적.
lstm_type = ['real', 'complex'] # not fullsubnet - LSTM - type
mask_type = ['Direct(None make)', 'E', 'C', 'R'] # not fullsubnet - masking mode
main_net = ['LSTM', 'GRU'] # fullsubnet

# experiment number setting
expr_num = 'AI_augmentation_final_exp01'
...
win_len = 400
win_inc = 100
ola_ratio = 0.75 # overlap Add Ratio
fft_len = 512
sam_sec = fft_len / fs
